...
import { Layout, Menu, Switch, Select, Button, Row, Col, List, Card } from 'antd';
...
export default function News() {
  ...
  useEffect(() => { setLoading(true); handleCallNews(); }, []);
  ...
  const handleCallNews = async () => {
    const url = 'https://corona--tracker.herokuapp.com/newslist?_page=1&_limit=12';
              ...
              renderItem={(item) => (
                <List.Item>
                  <a href={item.url
                    ...
                    <Card title={item.title} style={{ minWidth: 300, minHeight: 400, justifyContent: 'center' }} >
                      <img style={{ height: 100, width: 100 }} src={item.urlToImage} alt="Girl in a jacket" />
